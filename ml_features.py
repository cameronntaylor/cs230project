...
X_train, X_test, Y_train, Y_test = train_test_split(X_nv, Y, test_size=TEST_PER, random_state=RANDOM_STATE)
X_train, X_dev, Y_train, Y_dev = train_test_split(X_train, Y_train, test_size=DEV_PER/(1-TEST_PER), random_state=RANDOM_STATE)
...
# Base error
base_score1 = np.mean(np.abs(Y_test[:, WHICH_Y].mean()- Y_test[:, WHICH_Y]))
base_score2 = np.median(np.abs(Y_test[:, WHICH_Y].mean()- Y_test[:, WHICH_Y]))
base_score3 = np.median(np.abs(np.median(Y_test[:, WHICH_Y])- Y_test[:, WHICH_Y]))
...
for alpha in hp_grid_Lasso:
	...
	# Fit
	tune_Lasso.fit(X_train, Y_train[:,WHICH_Y])
	# Get score on dev set
	if WHICH_Y==0:
		tune_scores_Lasso[k] = tune_Lasso.score(X_dev, Y_dev[:, WHICH_Y])
	else:
		tune_scores_Lasso[k] = f1_score(Y_dev[:, WHICH_Y], (1.0*(model_Lasso.predict(X_dev)>=0.5)))
	k+=1
...
model_Lasso.fit(X_train, Y_train[:, WHICH_Y])

if WHICH_Y==0:
	final_score_Lasso = model_Lasso.score(X_test, Y_test[:, WHICH_Y])
	final_score_Lasso2 = np.mean(np.abs(model_Lasso.predict(X_test)- Y_test[:, WHICH_Y]))
	final_score_Lasso3 = np.median(np.abs(model_Lasso.predict(X_test)- Y_test[:, WHICH_Y]))
else:
	final_score_Lasso =  f1_score(Y_test[:, WHICH_Y], (1.0*(model_Lasso.predict(X_test)>=0.5)))
...
hp_grid_RF_d = np.unique(np.array(np.logspace(0,2,15, dtype="int64")))
...
if WHICH_Y==0:
	for j in range(hp_grid_RF.shape[0]):
		...
		tune_RF.fit(X_train, Y_train[:, WHICH_Y])
		tune_scores_RF[j] = tune_RF.score(X_dev, Y_dev[:, WHICH_Y])
		print(j)
	hp_RF_star = hp_grid_RF[np.where(tune_scores_RF==tune_scores_RF.max())][0]
	model_RF = RandomForestRegressor(
		n_estimators = hp_RF_star[0],
			max_depth=hp_RF_star[1])
	model_RF.fit(X_train, Y_train[:, WHICH_Y])
	final_score_RF = model_RF.score(X_test, Y_test[:, WHICH_Y])
	final_score_RF2 = np.mean(np.abs(model_RF.predict(X_test)- Y_test[:, WHICH_Y]))
	final_score_RF3 = np.median(np.abs(model_RF.predict(X_test)- Y_test[:, WHICH_Y]))
else:
	for j in range(hp_grid_RF.shape[0]):
		tune_RF = RandomForestClassifier(n_estimators = hp_grid_RF[j,0],
			max_depth=hp_grid_RF[j,1])
		tune_RF.fit(X_train, Y_train[:, WHICH_Y])
		tune_scores_RF[j] = f1_score(Y_dev[:, WHICH_Y], tune_RF.predict(X_dev))
	hp_RF_star = hp_grid_RF[np.where(tune_scores_RF==tune_scores_RF.max())][0]
	model_RF = RandomForestClassifier(n_estimators = hp_RF_star[0],
			max_depth=hp_RF_star[0])
	model_RF.fit(X_train, Y_train[:, WHICH_Y])
	final_score_RF = f1_score(Y_test[:, WHICH_Y], model_RF.predict(X_test))
